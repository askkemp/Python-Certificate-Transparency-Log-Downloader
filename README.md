# Introduction
A Python 3.11+ monolithic script to download RFC 6962 "classic" and [static tile](https://github.com/C2SP/C2SP/blob/main/static-ct-api.md]) from the [Certificate Transparency (CT) project](https://certificate.transparency.dev/) and extract specific X509 certificate metadata. The below is an example of the metadata captured for each certificate:
...
# Features
- Supports RFC 6962 "classic" and static tile CT logs
...
- For RFC 6962 logs, it attempts to download as many records as possible per query
...
PercentComplete  CurrentPosition   End(tree_size) LogType        CTProviderURL
            0.0                0             5881 tiled          https://mon.sycamore.ct.letsencrypt.org/2027h2/
            0.0                0        816987776 rfc6962        https://sphinx.ct.digicert.com/2026h1/
            0.0                0             4732 tiled          https://rennet2027h1.mon.ct.ipng.ch/
            0.0                0             5885 tiled          https://mon.willow.ct.letsencrypt.org/2027h2/
            0.0                0            17512 rfc6962        https://elephant2027h2.ct.sectigo.com/
            0.0                0        130815692 rfc6962        https://oak.ct.letsencrypt.org/2026h2/
...
```

### 6. Start the download
```bash
$ python3 ./download_ct_logs.py --ct_url https://mon.sycamore.ct.letsencrypt.org/2027h2/
```
...
## Recommended Use
Run one instance of the script for each CT log you want downloaded. I recommend ALWAYS chosing the tile CT log URL because it will download EXTREMELY faster versus the RFC 6962 logs. Reguardless of log choice, the results are real-time saved to disk and written to an S3 bucket during a log rotate. The CT download progress, as well as the max tree size, are maintained in DynamoDB.
...
usage: download_ct_logs.py [-h] [--init] [--show-all-status] [--update-tree-size] [--ct_url ARG_CT_URL] [--remove-progress-bar]
...
options:
  ...
  --remove-progress-bar
                        Removes showing the real-time progress bar.
```
...
- For RFC 6962 logs, the max number of records it will pull per HTTP request is based on the initial attempt at getting 1000 enteries and then falling back to the number that was actually provided. It is not dynamic.
...
## Related work
* [CertMonitor](https://github.com/dig-sec/CertMonitor "CertMonitor") - Created in 2025, Python that supports only RFC 6962 CT logs
* [Axeman](https://github.com/CaliDog/Axeman "Axeman") - Uses Python3 with concurrency and multi-processing to download CT logs. It has not been updated in three years. Supports only RFC 6962 CT logs.
* [CTLogInspector](https://github.com/rajivchocolate/CTLogInspector "CTLogInspector") - Uses Python 3 + DynamoDB, SQS, S3, and Lambda to download RFC 6962 CT logs.
